...
{   
   ...
   TH1D* h1 = new TH1D("h1", "Triplet Fit P3D = 2 GeV (with only hit uncertainties); p(fit) / p(gen); Entries;", 60, 0.75, 1.4);
   ...
   int Ev_Id;
   ROOT::Math::XYZPoint *coor0 = 0;
   ROOT::Math::XYZPoint *coor1 = 0;
   ROOT::Math::XYZPoint *coor2 = 0;
   double PHI_initial;
   ...
   treeIn -> SetBranchAddress("Coord0", &coor0);
   treeIn -> SetBranchAddress("Coord1", &coor1);
   treeIn -> SetBranchAddress("Coord2", &coor2);
   treeIn -> SetBranchAddress("In_Phi", &PHI_initial);
  ...
  // myFile.open("triplet_sim_out_true12.txt", ios::in);
   ...
   double     R3 = 150.0;
     ...
     {         
            ...
            /*bx_gen->GetEntry(tentry);
            by_gen->GetEntry(tentry);
            bz_gen->GetEntry(tentry);
            bPhi_gen->GetEntry(tentry);*/
             
             
            x0 = coor0->x(); y0 = coor0->y(); z0 = coor0->z();
            x1 = coor1->x(); y1 = coor1->y(); z1 = coor1->z();
            x2 = coor2->x(); y2 = coor2->y(); z2 = coor2->z();
            phi = PHI_initial; 
...
/*--------------------------------------------------------------------------------------------ENDCAP------------------------------------------------------------------------------------*/   
  ...
  // c2->Modified(); c2->Update();
   ...
   gr1->SetTitle("R-Rcal values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 20GeV");
   ...
   gr2->SetTitle("R-Rcal values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 20GeV");
   ...
   gr3->SetTitle("R-Rcal values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 20GeV");
   ...
   gr4->SetTitle("C2D values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 20GeV");
