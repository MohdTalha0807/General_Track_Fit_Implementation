...
//  }
  ...
  dz=PHI*r/tan(theta);                  // it is a relation between the arc length and the longitudinal distance z


  //update
  //cout<<"without hit uncertainty : "<<xnew<<"\t"<<ynew<<endl;
  
  xnew = calculate_hit_uncertainty(xnew); 
  ynew = calculate_hit_uncertainty(ynew); 
...
{   
   ...
   const char *path="/home/mtalha/Ph.D._work/Triplet_fit_sim/Triplet_fit_withHitUnc/MS_fit_constrained_hitUnc";    //Triplet_MS_Fit";
   ofstream fout_1;
   fout_1.open ("/home/mtalha/Ph.D._work/Triplet_fit_sim/Triplet_fit_withHitUnc/MS_fit_constrained_hitUnc/triplet_sim_out_true12.txt");
   //fout_1<<"x"<<"\t"<<"\t"<<"y"<<"\t"<<"\t"<<"z"<<endl;       
