...
// Triplet MS fit for the Unconstrained case 
...
{  
   ...
   TH1D* h1 = new TH1D("h1", "Triplet MS Fit P3D = 20 GeV (unconstrained with hit uncertainties); p(fit) / p(gen); Entries;", 100, 0.0, 5.0);
   TH1D* h2 = new TH1D("h2", "Triplet MS Fit; sigma_{p}(fit) [MeV]; Entries;", 200, 1000.0, 120000.0);
   TH1D* h3 = new TH1D("h3", "Triplet MS Fit (unconstrained with hit uncertainties); chi2_min_uncon; Entries;", 100, 0.0, 2.0);
   TH1D* h4 = new TH1D("h4", "Triplet MS Fit; (p(fit) - p(gen)) / sigma_{p}; Entries;", 120, -3.0, 3.0); 
   ...
   double C3D_min_uncon;
   double r, rt;
   double RhoPHI, RhoTheta;
   double chi2_min_uncon;
   double P3D;
   double sigma_MS;
   double sigma_C3D_uncon;
   double sigma_P3D;
   //double Theta_MS;
   //double PHI_MS;
      ...
      for (int x = 0; x < num_events; x++)
       {
	    myFile >> x0 >> y0 >> z0;
            ...
            C3D_min_uncon =  -(PHI0_tilde * PHI0_tilde * pow(sin((Theta1C + Theta2C) / 2.0),2) + Theta0_tilde * Theta0_tilde) / (RhoPHI * PHI0_tilde * pow(sin((Theta1C + Theta2C) / 2.0),2) + RhoTheta * Theta0_tilde);
            
            //cout<<C3D_min_con<<endl;
            
            P3D = 0.3 * B_avg / C3D_min_uncon;
            //cout<<P3D<<endl;
            ...
            b_MS = (45.0/B_avg) * sqrt(rel_mat_thickness);
            
            chi2_min_uncon = (1.0 / (b_MS * b_MS)) * pow(( RhoTheta * PHI0_tilde - RhoPHI * Theta0_tilde), 2) / ( PHI0_tilde * PHI0_tilde + Theta0_tilde * Theta0_tilde / pow(sin((Theta1C + Theta2C) / 2.0), 2));
            
            //sum_sigma += sigma_MS/ num_events;
             
            //sum_C3D += C3D_min_con/ num_events;
            
            //if(isnan(C3D_min_uncon))
            //{ cout<<Cc<<"      "<<xx<<"    "<<x0<<"    "<<x1<<"   "<<x2<<endl;
            //  cout<<y0<<"    "<<y1<<"   "<<y2<<endl;
            // }
             
            sum_chi2 += chi2_min_uncon / num_events;
            
            sigma_C3D_uncon = b_MS * pow((PHI0_tilde * PHI0_tilde * pow(sin((Theta1C + Theta2C) / 2.0),2) + Theta0_tilde * Theta0_tilde), 1.5) / pow((PHI0_tilde * RhoPHI * pow(sin((Theta1C + Theta2C) / 2.0),2) + Theta0_tilde * RhoTheta),2);
              
            sigma_P3D = P3D * sigma_C3D_uncon / C3D_min_uncon;
