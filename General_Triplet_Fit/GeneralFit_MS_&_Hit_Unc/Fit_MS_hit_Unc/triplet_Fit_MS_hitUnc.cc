...
double calculate_C3D_seed_MS(double Theta0_tilde, double RhoTheta, double PHI0_tilde, double RhoPHI, double Theta1C, double Theta2C)      // seed for the iterative solution for the curvature equation  (MS_dominated)
{
   double C3D_MS_min;
   
   C3D_MS_min = -(Theta0_tilde * RhoTheta + PHI0_tilde * RhoPHI * pow(sin((Theta1C+Theta2C)/2.0),2))/(RhoTheta * RhoTheta + RhoPHI * RhoPHI * pow(sin((Theta1C+Theta2C)/2.0),2));
 
 
 return C3D_MS_min;
}

double calculate_C3D_seed_hit(double Theta0_tilde, double RhoTheta, double PHI0_tilde, double RhoPHI, double Theta1C, double Theta2C, double gamma_trans2, double gamma_long2)      // seed for the iterative solution for the curvature equation  (MS_dominated)
{
   double C3D_hit_min;
...
{  
   ...
   double phi; 
   double theta;
   double b_MS = (45.0/B_avg) * sqrt(0.01);     // x/x0 = 0.01  
   ...
   cout<<"number of entries in TTree = "<<nentries<<endl;
      ...
      for (int l = 0; l < nentries; l++)
     {
	    treeIn -> GetEntry(l);
            ...
            sigma_MS = abs(C3D_cal) * b_MS;
            
            C3D_min = - (Theta0_tilde * RhoTheta * (pow(sigma_MS,2) + gamma_trans2*pow(sin((Theta1C+Theta2C)/2.0),2)) + PHI0_tilde * RhoPHI * (pow(sigma_MS,2) + gamma_long2)*pow(sin((Theta1C+Theta2C)/2.0),2)) / (RhoTheta * RhoTheta * (pow(sigma_MS,2) + gamma_trans2*pow(sin((Theta1C+Theta2C)/2.0),2)) + RhoPHI * RhoPHI * (pow(sigma_MS,2) + gamma_long2)*pow(sin((Theta1C+Theta2C)/2.0),2));
            
            chi2_3D_min = pow((Theta0_tilde * RhoPHI - PHI0_tilde * RhoTheta), 2) / (RhoTheta * RhoTheta * (pow(sigma_MS/sin((Theta1C+Theta2C)/2.0),2) + gamma_trans2) + RhoPHI * RhoPHI * (pow(sigma_MS,2) + gamma_long2));
            
            sigma_C3D = sqrt( ((pow(sigma_MS,2) + gamma_long2) * (pow(sigma_MS,2) + gamma_trans2*pow(sin((Theta1C+Theta2C)/2.0),2))) / (RhoTheta * RhoTheta * (pow(sigma_MS,2) + gamma_trans2*pow(sin((Theta1C+Theta2C)/2.0),2)) + RhoPHI * RhoPHI * (pow(sigma_MS,2) + gamma_long2) * pow(sin((Theta1C+Theta2C)/2.0),2)) );
