...
int main()
{
  ...
  { 
   theta = M_PI/6 + rand1->Rndm() * (M_PI/2.0 - M_PI/6);
   phi = rand2->Rndm() * 2.0 * M_PI;      // all generated in the units of mm
   x = -2.0 + rand3-> Rndm() * 4.0;
   y = -2.0 + rand4-> Rndm() * 4.0;
   z = -50.0 + rand5-> Rndm() * 100.0;
   
   fout<<x<<"\t"<<y<<"\t"<<z<<"\t"<<phi<<"\t"<<theta<<endl; 
  }
