...
double calculate_hit_uncertainty(double x_coord, double pixel_width)
{
  double dx;    
...
vector<double> cross_layer(double x, double y, double z, double r, double theta, double phi, double R, int distantpoint)  // including the hit uncertainties
{
  ...
  if (PHI1<PHI2) {
    if (distantpoint==0) {
      ...
      xnew=xnew1;
      ynew=ynew1;
      phi_wc = alpha1;                // i have defined for my personal use  // this is phi_hit
    }else{
      PHI=PHI2;
      xnew=xnew2;
      ynew=ynew2;
...
{   
   ...
   vector<int> Ev_Id;
   vector<double> Co_X, Co_Y, Co_Z, PHI_initial, theta_initial;
   int particle_id = 0;
   ...
   treeOut -> Branch("CoordZ", &Co_Z);
   treeOut -> Branch("In_Phi", &PHI_initial);
   treeOut -> Branch("In_Theta", &theta_initial);
      
   double q = 1.0;                                     
   double phi, x,y,z, In_phi;                      
   double p3d = 1000.0;     // in MeV   
   ...
   vector<double> phi1_vec, phi2_vec, phi3_vec;
   vector<double> vec_Xc;
   vector<double> vec_Yc;
   vector<double> vec_Zc;
   
   double phi_without_unc, phi_with_unc;
   
   
   double Cc, d01, d02, d12;
   double x_hitUnc, y_hitUnc, z_hitUnc;
   double dx, dy, du, dv, da;
   double pixel_widthX;
   double pixel_widthY;
   double pixel_widthZ;
   double phi_before, phi_after;
   double theta_ini;
   ...
   {  int counter = 0;
      int i = 0;
    while( myFile >> x >> y >> z >> phi >> theta && counter < 10000)    // 'counter' is for the number of particle simulations one wants to see.
    {  
    ...
    for(int j = 1; j <= 3; j++)
    {   
     R = j*100.0;            // the radius of the barrel layer
    ...
    //cout<<R<<""<<R_cal<<endl;
     ...
     phi_without_unc = atan2(y,x); 
     phi_before = phi_without_unc * 180.0/PI;
     //while (phi_without_unc *180.0/PI < 0) 
     //{phi_without_unc+=2*PI;}                                 
     ...
     dv = 0.0; 
     pixel_widthX = 0.1/sqrt(12.0);
     pixel_widthY = 0.1/sqrt(12.0); 
     pixel_widthZ = 0.1/sqrt(12.0);
                                         
     da = calculate_hit_uncertainty(du, pixel_widthX);
     dv = calculate_hit_uncertainty(dv, pixel_widthZ);      
     dx = -da * sin(phi_wc);   
     dy =  da * cos(phi_wc);
     x_hitUnc = x + dx;
     y_hitUnc = y + dy;
     ...
     //cout<<dx<<"    "<<da<<endl;
     //cout<<x_hitUnc<<endl;
     //x_hitUnc = x_hitUnc * R / sqrt(x_hitUnc*x_hitUnc+y_hitUnc*y_hitUnc);
     //y_hitUnc = y_hitUnc * R / sqrt(x_hitUnc*x_hitUnc+y_hitUnc*y_hitUnc);
     //cout<<x_hitUnc<<"      "<<R / sqrt(x_hitUnc*x_hitUnc+y_hitUnc*y_hitUnc)<<endl;
     //cout<<"---------------------------"<<endl;
     
     phi_with_unc = atan2(y_hitUnc,x_hitUnc);
     phi_after = phi_with_unc * 180.0/PI;
     //while (phi_with_unc *180.0/PI < 0) 
     //{phi_with_unc+=2*PI;}
      
      if(R == 100.0)
    { phi1_vec.push_back((phi_without_unc - phi_with_unc) * 180.0 / PI);
      if(abs((phi_without_unc - phi_with_unc) * 180.0 / PI) > 300.0)
      { cout<<x<<"    "<<y<<endl;
        cout<<x_hitUnc<<"    "<<y_hitUnc<<endl;
        cout<<phi_before<<"    "<<phi_after<<endl;
        cout<<phi_without_unc * 180.0 /PI<<"    "<<phi_with_unc* 180.0 /PI<<endl;
        cout<<"---------------------------"<<endl; }
     }
    if(R == 200.0)
    { phi2_vec.push_back((phi_without_unc - phi_with_unc) * 180.0 / PI); }
    if(R == 300.0)
    { phi3_vec.push_back((phi_without_unc - phi_with_unc) * 180.0 / PI); }
    
     
     Ev_Id.push_back(particle_id);
     Co_X.push_back(x_hitUnc);
     Co_Y.push_back(y_hitUnc);
     Co_Z.push_back(z_hitUnc);
     PHI_initial.push_back(phi_ini * 180.0 / PI);
     theta_initial.push_back(theta_ini * 180.0 / PI);   //change this
...
// -------------------------------visualizing the hits on the barrel layers----------------------------------------
   ...
   } // closing of the inner loop finding the three hits on the barrel layers 
...
//------------------------------------------------- for endcap define the z positions of the first three layers----------------------------------------------------// 
   ...
   }   // closing of the outer loop going over the simulated particles
     
     myFile.close();   // closing of the text file containing the initial (xyz, phi) parameters of the simulated particles 
   } 
...
/*--------- -----------------------------------------------------------Visualization of ENDCAP layers------------------------------------------------------------------------------------*/   
...
//---------------------------------------------------------------- Control plots -------------------------------------------------------------   
   ...
   */
   ...
   g1->SetTitle("R-Rcal values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 1GeV");
   ...
   g2->SetTitle("R-Rcal values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 1GeV");
   ...
   g3->SetTitle("R-Rcal values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 1GeV");
   ...
   g->SetTitle("Particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 1GeV");
   ...
   c6->cd(1);   
   TGraph *t1 = new TGraph(sz, &phi_vec[0], &phi1_vec[0]);
   t1->SetTitle("#phi-#phi^{,} values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 1GeV");
   t1->GetYaxis()->SetTitle("#phi-#phi^{,} [degrees]");
   t1->GetXaxis()->SetTitle("#phi [degrees]");
   //gr1->GetYaxis()->SetLimits(0.0,150.0);
   //gr1->GetXaxis()->SetLimits(-1.0, 1.0);
   ...
   c6->cd(2);   
   TGraph *t2 = new TGraph(sz, &phi_vec[0], &phi2_vec[0]);
   t2->SetTitle("#phi-#phi^{,} values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 1GeV");
   t2->GetYaxis()->SetTitle("#phi-#phi^{,} [degrees]");
   t2->GetXaxis()->SetTitle("#phi [degrees]");
   //gr1->GetYaxis()->SetLimits(0.0,150.0);
   //gr1->GetXaxis()->SetLimits(-1.0, 1.0);
   ...
   c6->cd(3);   
   TGraph *t3 = new TGraph(sz, &phi_vec[0], &phi3_vec[0]);
   t3->SetTitle("#phi-#phi^{,} values for different #phi for particles simulated with polar angle #frac{#pi}{3} and P_{3D} = 1GeV");
   t3->GetYaxis()->SetTitle("#phi-#phi^{,} [degrees]");
   t3->GetXaxis()->SetTitle("#phi [degrees]");
   //gr1->GetYaxis()->SetLimits(0.0,150.0);
   //gr1->GetXaxis()->SetLimits(-1.0, 1.0);
